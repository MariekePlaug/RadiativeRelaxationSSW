...
class AtmosphericFlux:
    ...
    def __init__(
        ...
        remove_lines_percentile: (
            dict[pyarts.arts.SpeciesEnum, float] | float | None
        ) = None,
    ):
    ...
    def __call__(
        ...
        Args:
            atmospheric_profile (xarray): A dictionary of atmospheric data. Defaults to {}.
            surface_temperature (float, optional): A surface temperature. Defaults to None.
            atm_latitude (float, optional): Atmospheric latitude. Defaults to 0.0.
            atm_longitude (float, optional): Atmospheric longitude. Defaults to 0.0.
            max_level_step (float, optional): Maximum layer thickness [m]. Defaults to 1e3.
        Returns:
        ...
        self.ws.atmospheric_field = pyarts.data.to_atmospheric_field(
            atmospheric_profile
        )
        ...
        self.SOLAR = Flux(
            "solar",
            np.einsum("i,ik->k", self.visw, self.ws.disort_spectral_flux_field.up),
            np.einsum(
                "i,ik->k", self.visw, self.ws.disort_spectral_flux_field.down_diffuse
            ),
            np.einsum(
                "i,ik->k", self.visw, self.ws.disort_spectral_flux_field.down_direct
            ),
        )
        ...
        self.THERMAL = Flux(
            "thermal",
            np.einsum("i,ik->k", self.ir_w, self.ws.disort_spectral_flux_field.up),
            np.einsum(
                "i,ik->k", self.ir_w, self.ws.disort_spectral_flux_field.down_diffuse
            ),
            np.einsum(
                "i,ik->k", self.ir_w, self.ws.disort_spectral_flux_field.down_direct
            ),
        )
